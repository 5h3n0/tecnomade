    ...
    <section class="screen_exibition_ini">
        ...
        <section class="bloco_contrate_profissionais">
            ...
            <div class="box_text_ctrtPf">
                <h3>Contrate Profissionais <br> de confiança para seus problemas</h3>
                <div class="bloco_de_texto">
                    <p>Quando se trata de lidar com problemas, a confiança é fundamental. É por isso que oferecemos a
                        você a oportunidade de contratar profissionais de confiança para resolver seus problemas. Nossa
                        equipe é composta por especialistas altamente qualificados e dedicados, prontos para enfrentar
        ...
        <section class="bloco_seja_profissional">
            ...
            <img src="./imgs/import_614eb638cdee05.43942843_720p_5000br.gif" class="img_ctrtPf" alt="Img_sjPf">

            <div class="box_text_ctrtPf">
                <h3>Seja um Profisional da Tecnomade</h3>
                <div class="bloco_de_texto">
                    <p> Nossa plataforma oferece uma oportunidade para os especialistas em tecnologia se destacarem,
                        conectando-os diretamente com pessoas e empresas que precisam de suas habilidades. Se você é um
                        desenvolvedor, designer, analista de dados ou especialista em qualquer campo relacionado à
                        tecnologia, a Tecnomade é o lugar certo para você. Mostre suas habilidades, construa sua
        ...
        <section class="conteiner_perfils" style="display:flex;">

            

        <?php
// Fetch professionals with average ratings between 4 and 5
$sql = "
    SELECT p.*, AVG(a.estrelas) AS average_rating
    FROM prof p
    INNER JOIN avaliacoes a ON p.id_Pf = a.id_Pf
    GROUP BY p.id_Pf
    HAVING average_rating BETWEEN 4 AND 5
    ORDER BY RAND()
";
...
<?php foreach ($professionals as $professional): ?>
    <div class="box__perfil" data-id="<?= $professional['id_Pf'] ?>">

        <img src='<?= "upload/" . $professional['imgName'] ?>' alt="Imagem do Profissional" class="img_perfil_of_box">
        <h3 class="name_of_box_perfils">
            <?= $professional['pfName'] ?>
        </h3>

        <div class='orderEstrelas' style='display:flex;'>
            <?php
            $average_rating = number_format($professional['average_rating'], 2);
        ...
        $catStmt->bind_param('i', $professional['id_Pf']);
        $catStmt->execute();
        ...
        while ($catRow = $catResult->fetch_assoc()) {
            echo "<p class='descrytion_of_box_perfils'>{$catRow['nome']}</p>";
            $counter++;
        }
